 ...
 */
int md5_read_file(const char *filename, char *sig_buf, int sig_buf_size)
{
	...
	while (1) {
		...
		ret = fread(sig_buf, sizeof(char), sig_buf_size, stream);
		if (ret <= 0)
			break;

		/* process our sig_buf sig_buf */
		md5_process(&md5, sig_buf, ret);
	}
	...
	md5_sig_to_string(sig, sig_buf, sig_buf_size);
